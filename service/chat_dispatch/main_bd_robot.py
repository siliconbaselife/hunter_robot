...
class MainBDRobot(BaseChatRobot):
    ...
    def contact(self, page_history_msg, db_history_msg):
        ...
        try:
            ...
            company_infos = self.fetch_company_infos()

            logger.info("company_infos: ", company_infos)
            ...
            r_msg, action = self.generate_reply(intention, status_infos, history_msgs, reply_infos, company_infos)
    ...
    def fetch_company_infos(self):
        return self.job_config["company_infos"]

    def generate_reply(self, intention, status_infos, history_msgs, reply_infos, company_infos):
        ...
        if intention == INTENTION.POSITIVE:
            return self.positive_reply(status_infos, reply_infos, history_msgs, company_infos)

        if intention == INTENTION.NOTHING:
            return self.nothing_reply(status_infos, reply_infos, history_msgs, company_infos)
    ...
    def nothing_reply(self, status_infos, reply_infos, history_msgs, company_infos):
        m = ""
        if "contact_flag" in status_infos and status_infos["contact_flag"]:
            m = "找机会要客户联系方式, 要联系方式的话术是 我们加个微信细聊一下呗$PHONE$"
        prompt = f'''
你是一个BD，你在跟客户推销你们公司的业务。
不能说重复的话术。
回答在30个字以内。
用户要联系方式，我晚一点儿加你吧。
{m}
...
'''
    ...
    def negtive_reply(self, status_infos, reply_infos):
        ...
        if "intention" not in status_infos and "negtive_msg" not in reply_infos:
            status_infos["intention"] = False
            status_infos["contact_flag"] = True
            negtive_msg = "我们加个微信或者交换一下电话认识一下呗，未来可以保持长期联系。"
            return negtive_msg, ChatStatus.NeedContact

    def positive_reply(self, status_infos, reply_infos, history_msgs, company_infos):
        if "intention" in status_infos:
        ...
        if "intention" not in status_infos and "positive_msg" not in reply_infos:
            status_infos["intention"] = True
            status_infos["contact_flag"] = True
            positive_msg = "如果有需求的话，我们加个微信或者交换一下电话，具体认识沟通一下呗。"
...
'''
    ...
    def deal_intention_prompts(self, history_msgs):
        ...
        prompt = f'''
你是一个猎头公司的销售，判断一下，下面的对话，用户最后一段对话的意图。
历史对话在分隔符###里面，客户最后一段对话在@@@里面
A.有需求 B.没有需求 C.无法判断
###
{msg_str}
###
