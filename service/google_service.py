...
from dao.tool_dao import create_google_account, query_google_account, delete_google_account, get_google_account, \
    get_resume_by_candidate_ids_and_platform
...
def create_account(manage_account_id, user_info, credentials):
    logger.info(
        "[backend_tools] create account by manage_account_id = {}, user_info = {}, credentials".format(
            manage_account_id, user_info, credentials)
    )
    ...
    create_google_account(openid=id, manage_account_id=manage_account_id, name=name, picture=picture,
                          google_account_email=email, credentials=credentials)
...
def revoke_token(token):
    ...
    logger.info(
        "[backend_tools] revoke_token by token = {} response status_code = {} text = {}".format(token,
...
def create_message(sender, to, subject, message_text):
    logger.info(
        "[backend_tools] create_message by sender = {}, to = {}, subject = {}, message_text = {}".format(sender, to,
                                                                                                         subject,
                                                                                                         message_text)
    )
...
def send_message_by_gmail(manage_account_id, openid, platform, candidate_id, title, content, email_to):
    logger.info(
        "[backend_tools] send_message by manage_account_id = {}, openid = {}, platform = {}, candidate_id = {}, title = {}, content = {}, email_to = {}".format(
            manage_account_id, openid, platform, candidate_id, title, content, email_to)
    )
...
def send_email_content_raw(manage_account_id, platform, candidate_id, email_title, content, email_type, openid):
    ...
    try:
        ...
        # logger.info(f"send_email_content_raw => send_email_content_raw {type(raw_profile)} rows: {raw_profile}")
