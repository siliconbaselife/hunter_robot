from dao.contact_bank_dao import new_contact, new_extension_user, query_user_credit, query_user_already_contacts, update_user_credit, update_user_already_contacts, query_contact_by_profile, update_contact_personal_email, update_contact_phone
...
def user_fetch_contact(user_id, linkedin_profile, contact_tag):
    ...
    credit = fetch_user_credit(user_id=user_id)
    if credit < price:
        logger.info(f'extension user {user_id} credit {credit} insufficient for fetch {contact_tag}, which need {price}')
        return None,  f"credit insufficient"
    need_update_credit = True
    ...
    if not db_content:
        logger.info(f'extension user {user_id} need contact({contact_tag}) for {profile}, db not found, need fetch from outside')
        ## no information in db, need query contactout
        ...
        ## update db
        if db_item_exists:
            logger.info(f'extension user {user_id} need contact({contact_tag}) for {profile}, got outside {res}, now will update contact')
            ctx['update_db_func'](profile, res)
        else:
            logger.info(f'extension user {user_id} need contact({contact_tag}) for {profile}, got outside {res}, now will new contact')
