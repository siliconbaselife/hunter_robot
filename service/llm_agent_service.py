    ...
    def __init__(self):
        ...
        prompt = PromptTemplate(
            ...
            template='{profile}\n'
                     ...
                     'function_type => 区分简历的职能，先判断简历是属于前台、中台还是后台, 然后判断属于相应哪个枚举值, 只能返回是某个枚举值，前台枚举值 {befores}, 中台枚举值 {middles}, 后台枚举值 {afters}, 如果无法判断，给出 "无法判断" 的字段。\n'
                     'service_country => 服务的国家，如果无法判断，给出 "无法判断" 的字段。\n'
                     'rank_of_position => 根据简历判断一下，他的岗位级别，如果无法判断，给出 "无法判断" 的字段。枚举值有 '
                     '"Specialist" "Supervisor" "manager" "director" "General Manager " "Vice President " "CXO" "无法判断" '
        )
    ...
    def __init__(self):
        ...
        prompt = PromptTemplate(
            ...
            template='{profile}\n'
                     ...
                     '以上是一个候选人在linkedin的简历。请帮我对该简历进行分析，并返回json格式。\n'
                     '需要萃取出的字段如下\n'
                     'age => 通过学校毕业的时间，大学 + 22，研究生 + 25，高中 + 18。如果无法判断，返回 "未知"\n'
                     'work_year => 工作年限，通过学校毕业的时间进行推测，返回数字。如果无法判断，返回 "未知"'
