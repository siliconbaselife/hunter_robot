...
from utils.utils import is_211, is_985, str_is_none
...
def liepin_autoload_filter_v2(candidate_info, job_res):
    ...
    if degree == '':
        ...
        degree_ok = degree_compare_v2(degree, min_degree)

    school_ok = False
    for edu in candidate_info.get('eduExpFormList', []):
        if school_threshold == 2:
            if is_985(edu['redSchool']):
                school_ok = True
        elif school_threshold == 1:
            if is_211(edu['redSchool']):
                school_ok = True
        else:
            school_ok = True
    ...
    if 'ex_company' in filter_args and filter_args['ex_company'] != "":
        ...
        if len(ex_company) > 0:
            ex_company_ok = False
            for c in ex_company:
                for w in candidate_info.get('workExps', []):
                    if c in w['rwCompname'] or w['rwCompname'] in c:
                        ex_company_ok = True
