...
def judge_and_update_force(account_id, filter_result):
    ...
    with _account_force_lock:
        ...
        if to_touch_strategy:
            logger.info(
                f"judge_and_update_force: account {account_id} to strategy touch, count {_account_force_context[account_id]['count']} will clear")
            _account_force_context[account_id]['count'] = 0
        elif _account_force_context[account_id]['count'] >= _account_force_thresh:
            logger.info(
                f"judge_and_update_force: account {account_id} meet count thresh, count {_account_force_context[account_id]['count']} will force")
            _account_force_context[account_id]['count'] = 0
            force_touch = True
    if force_touch:
