...
from service.tools_service import query_profile_id_tag_v3
...
def new_job_service_v3(manage_account_id, platform_type, dynamic_job_config, template_config, job_id, platform_id):
    ...
    job_config_json = json.dumps(job_config, ensure_ascii=False)
    logger.info(
        f'new_job_service_v3: {platform_type} {platform_id} {job_name} {robot_api} {job_config_json}, {share}, {manage_account_id},{robot_template}')
...
def chat_list_service(job_id, begin_time, end_time, page, limit, with_phone, with_wechat, with_reply, with_resume):
    return_list = []
    candidate_list = chat_parse(job_id, begin_time, end_time, page, limit, with_phone, with_wechat, with_reply,
                                with_resume)
...
def stat_chat_service(job_id, begin_time, end_time, page, limit, with_phone, with_wechat, with_reply, with_resume):
    ...
    for item in candidate_list:
        source, cv, phone, wechat, user_reply, chat_info_list = item['source'], item['cv'], item['phone'], item[
            'wechat'], item['reply'], item['chat']
        if cv is not None:
            with_cv_cnt += 1
...
def chat_parse(job_id, begin_time=None, end_time=None, page=None, limit=None, with_phone=False, with_wechat=False,
               with_reply=False, with_resume=False):
