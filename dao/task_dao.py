...
sql_dict = {
    ...
    "query_candidate": "select details from candidate where candidate_id='{}'",
...
def register_job_db(job_name, job_jd, robot_api):
    job_id = str(uuid.uuid1())
    # d = [[job_id, job_name, job_jd, robot_api]]
    dbm.insert(sql_dict['register_job'].format(job_id, job_name, job_jd, robot_api))
    return job_id
...
def hello_exec_db(account_id, job_id, exec_date):
    dbm.update(sql_dict["add_task_count"].format(account_id, job_id, exec_date))