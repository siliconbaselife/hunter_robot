...
def insert_llm_template(manage_account_id, template_id, template_name, template_config):
    ...
    template_config = template_config.replace('\"', '\\"')
    return dbm.insert(
        sql_dict['insert_llm_template'].format(manage_account_id, template_id, template_name, template_config))
...
def select_extention_config(manage_account_id, account_id):
    sql = f"select extention_config from account where manage_account_id = '{manage_account_id}' and account_id = '{account_id}'"
    data = dbm.query(sql)
    if len(data) == 0:
        return None
