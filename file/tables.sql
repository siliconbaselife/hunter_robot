...
alter table manage_account add `account_num` int unsigned NOT NULL DEFAULT 0 COMMENT '账号数量' after `desc`;
alter table manage_account add `invite_account` varchar(100) NOT NULL DEFAULT "" COMMENT '账号' after `account_num`;
...
CREATE TABLE IF NOT EXISTS `resume_list`(
   ...
   `manage_account_id` VARCHAR(100) NOT NULL COMMENT '管理账户ID',
   `platform` VARCHAR(50) NOT NULL COMMENT '平台',
   `list_name` VARCHAR(50) NOT NULL COMMENT '列表名称',
   `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
   PRIMARY KEY ( `id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_as_cs;

CREATE TABLE IF NOT EXISTS `resume_list_relation`(
   `id` bigint(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
   `manage_account_id` VARCHAR(100) NOT NULL COMMENT '管理账户ID',
   `list_name` VARCHAR(50) NOT NULL COMMENT '平台',
   `candidate_id` VARCHAR(100) NOT NULL COMMENT '候选人ID',
   `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
   PRIMARY KEY ( `id`)
...
CREATE TABLE `company_tag` (
  ...
  `company_id` VARCHAR(255) NOT NULL COMMENT '公司linkedin链接',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `tag_company_id` (`tag`,`company_id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_as_cs;

CREATE TABLE `company_info` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `company_id` VARCHAR(255) NOT NULL COMMENT '公司linkedin链接',
  `linkedin_doc` LongText COMMENT 'linkedin document',
  `wiki_doc` LongText COMMENT 'wiki document',
  `google_search_doc` LongText COMMENT 'google search document',
  `pdf_doc` LongText COMMENT 'pdf document',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `warehouse` (
  ...
  `Larehouse_Duration` VARCHAR(20) NOT NULL,
...
CREATE TABLE `staff_manage` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `manage_account_id` varchar(100) NOT NULL COMMENT '管理账户ID',
  `account_manage_id` varchar(100) NOT NULL COMMENT '员工账户ID',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
