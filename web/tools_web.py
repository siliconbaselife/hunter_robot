...
def send_email_contents_web():
    ...
    # type: 'google_account' | 'smtp'
    type = request.json.get('type', 'smtp')
...
def delete_profile_tag():
    ...
    if cookie_user_name == None:
        ...
        manage_account_id = decrypt(cookie_user_name, key)
    logger.info(f"delete_profile_tag manage_account_id: {manage_account_id} candidate_id: {candidate_id} tag: {tag}")
    delete_profile_tag_service(manage_account_id, candidate_id, platform, tag)
...
def data_to_excel_file(file_path, titles, data):
    try:
        ...
        worksheet.set_column(0, 0, 40, row_formatter)
        worksheet.set_column(1, 1, 30, row_formatter)
        worksheet.set_column(2, 2, 20, row_formatter)
        worksheet.set_column(3, 3, 50, row_formatter)
        worksheet.set_column(4, 4, 30, row_formatter)
        worksheet.set_column(5, 5, 20, row_formatter)
        worksheet.set_column(6, 6, 70, row_formatter)
